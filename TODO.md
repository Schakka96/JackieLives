...
> *"he grunts EVERY time when V hits a dialogue selection — make it a rare occurrence. I kinda like
> it tho, so a small chance (15%) is ok. He throws an arrival line (time we were on our way for
> example) EVERY time V returns to the main dialogue picker hub, not good. Also, there's not much
> small talk rn, can you author something from the existing library? Also he does not apply the
> correct male/female V versions, the subtitles now say chica (husbando mode) but his voice says
> mano often."*

…and, mid-session: *"jackie has ahi luego V lines but they don't play with vo after dismissing - why?"*

Five reports. Two were tuning. Three were the same root cause wearing different clothes: **we were
still writing content as if v1.66 hadn't happened.**

### 1. The grunt on every selection → a 15% roll
`Config.voice.gruntChance = 0.15`, honoured in `vo.lua`'s fallback. The vocal effort was designed for
when an unvoiced line was the exception; once the hub became mostly written text, "unvoiced" was the
COMMON case and every menu pick answered with a grunt. `1.0` restores the old behaviour, `0` disables
it without emptying `gruntPool`.

### 2. The hub re-greeting → `greetOnce`
Every topic in the `everywhere` hub ends `to = "open"`, and `open`'s pool is his ARRIVAL greetings —
so one conversation played as six hellos. `Branch.start` now speaks a node's line the FIRST time it is
entered in a conversation and goes straight to the choices on the way back in; `bstate.seen` is the
ledger, dropped with `bstate.taken`. Set on the main hub and the seated-dinner hub. `test_vo.lua`
walks every tree, finds any start node that sits on a cycle, and fails if it lacks the flag — so a
new looping tree can't reintroduce this.

### 3. "Ahí luego, V." never spoke — a TODO that outlived its problem
It sat in `Config.dismiss.partingPool` as `sfx = nil` behind a note asking for a WolvenKit scrape into
the Audioware bank. v1.66 made that job unnecessary and nobody deleted the note. **There was never
anything to scrape** — the line is in the game and naming its String ID is all it ever needed. There
are FOUR clean takes (plus "Hasta luego."), so the send-off now varies in delivery, not just in words.

> **The general lesson, and the one worth remembering:** after v1.66, "we don't have a clip for that"
> is almost never true. If Jackie ever said it, we can play it. Any `sfx = nil` with a note about
> extracting audio is stale by definition — look the line up instead.

### 4. Male/female V — the mod was arguing with the engine, and losing
The subtitle said "chica" while the audio said "mano". Not a bug in the Husbando/Hermano switch — a
bug in believing that switch had anything to do with it.

CDPR recorded some Jackie lines TWICE under **one String ID**, and the game picks the take from V's
**body gender** before a mod gets a vote (`docs/research/native_vo_dialogline.md` Q6 said as much:
"nothing for us to do" — true of the AUDIO, and we read it as true of the subtitle too). Our subtitle
is a hard-coded string written from the female take. Male-bodied V → male audio, female subtitle.

* **`mod/JackieLives/vo_gender.lua`** (generated by `tools/gen_vo_gender.py` from the game's own
  subtitle table) holds both takes per id. `jlLineText` swaps the subtitle for a male V, at the single
  chokepoint `speakJackieLine`, so hub/barks/calls/greetings are all covered at once.
* **Female V is left on the authored text on purpose** — it already IS the female take, and it is what
  `translations.lua` is keyed on. Swapping in CDPR's punctuation would drop nine languages to English
  to fix nothing.
* **`Config.hermanoLines` is DELETED and must not come back.** It rewrote VOICED lines by the
  relationship switch — the one case where the game owns the words. Worse, its "male mirror clips"
  were WEM STEMS (`jl_jackie_q003_m_18ac…`), and `vo.lua` only speaks `jl_<digits>`, so **the male
  track was the only track in the mod that couldn't talk at all.** Converting each stem's trailing hex
  to decimal gave the real String ID — and every one turned out to be the SAME line as its "female"
  counterpart. The duplicate pools (`arrivalGreetingsM`, `greetingsM`, `partingPoolM`) are gone; two
  lines that only existed in them were rescued into the shared pools.

**The rule now, stated once so it doesn't have to be rediscovered:** *if a line has an `sfx`, the game
owns the words — we only choose WHICH line. If it doesn't, we own them.* Inline `m = {...}` is still
correct, for text-only lines only.

### 5. Small talk — eleven new topics, and most of them he SPEAKS
Everything written for the hub in v1.65 was silent (`muteFallback`), which is why the grunt was so
conspicuous. The new topics are **cast, not written**: real CDPR recordings found in the local install
with NCLives' `tools/build_line_library.py`, quoted verbatim so the subtitle IS the audio.

`drink` · `city` · `misty2` · `iguana` · `fixer` · `padre` · `code` · `faith` · `death`, tiered across
familiarity like the existing ones, plus a hub draw widened from 3–4 to 4–5 rows. Written lines still
carry the deep tiers, so the FIRST answer to a new question is nearly always his real voice.

⚠️ **Casting, not writing.** Half the good-sounding lines in the library are him pitching a gig in
2077, and using those would quietly rewrite the character back into the man who died. Rejected on
those grounds, for the record: *"We got a new job lined up"*, *"Gonna be up to our necks in juicy
contracts"*, *"Then maybe we'll make some heavy money."*

### Tooling + housekeeping
| | |
|---|---|
| `tools/gen_vo_gender.py` | generates `vo_gender.lua`. Re-run after adding voiced lines. |
| `tools/gen_vo_durations.py` | now falls back to the line library for lines SoundDB places in no scene — **84/84 ids have an exact duration**, up from 59/63. |
| `tools/lang_apply.py` | splices translations into `translations.lua` and prunes dead keys, instead of hand-editing nine parallel blocks. |
| `tools/lang_extract.py` | harvests `vo_gender.lua` too. **All 9 languages back to 100%** (58 new strings each). |
| `tools/test_vo.lua` | 81 → 119 checks: the chance gate, the gender table, tree reachability, and "every voiced line is a real line of the game's". |
| `tools/test_familiarity.lua` | its "no tier-gated line may claim a clip" rule was an Audioware-era rule and is inverted: tier-gated lines MAY be voiced, and every id must exist. |

### Still open
* **Voiced subtitles are ours, not the game's.** We draw English text and translate it ourselves, but
  the game already holds every line in the player's own language, keyed by the same String ID. Reading
  it at runtime would make every voiced line correctly localized for free — and would have made the
  chica/mano bug impossible. Worth a look; not attempted here.
