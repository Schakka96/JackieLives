...
SOURCES = {
    ...
    "session.lua":   (),
    # v1.69, generated by tools/gen_vo_gender.py: the male-V wording of the lines CDPR recorded
    # twice under one String ID. Only the `m` values are ever put on screen (jlLineText leaves
    # female V on config.lua's authored text, which is already translated), so `f` is harvested
    # too but simply matches a key translations.lua already carries.
    "vo_gender.lua": ("f", "m"),
}
