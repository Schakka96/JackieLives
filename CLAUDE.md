...
| `mod/JackieLives/zengine.lua` | OPTIONAL **0-Engine** integration (Nexus 27967), global `ZEngine` — take the mod stack's shared per-frame state instead of polling beside it. Automatic, no player switch; ported from NCLives v1.78. ⚠️ Read `../NCLives/docs/research/zero_engine.md` first: never move our tick into their `OnUpdate` (the main-menu ticks must survive), never source V's POSITION from them (cross-mod tick order = a one-frame follow drift), and `tier == 0` means "not polled yet", not "gameplay". Every accessor answers **nil = ask the engine yourself**, never a guess. Note this repo has no `Perf` frame cache, so here it replaces an UNCACHED read — `jlInCutscene` was paying three cross-boundary calls per call. |
...
| `lua tools/test_zengine.lua` | The OPTIONAL **0-Engine** adapter, **54 checks**, both paths weighted equally: absent (the normal case) · attached · too old · layout changed · throwing mid-session · switch off · unbound. Run after touching `zengine.lua`. |
